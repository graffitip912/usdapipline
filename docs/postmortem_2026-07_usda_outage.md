...
7. **export_sales = FAS OpenData API 서버측 500** (2026-07-02~03 지속, 키 정상): www.usda.gov와 별개의 외부 장애. 수집기는 우아하게 저하하나 graceful skip 시 manifest에 시도 기록이 없어 상태가 never_run으로 표시됨. FAS 서버 복구 후 재실행하면 자동 정상화
   - **2026-07-03 심층 조사 결과**: ESRQS 신규 시스템(2026-03 런칭, 레거시 조회 사이트 2026-06-02 폐기)은 보고자용 포털이며, 프로그램 접근용 공식 API는 여전히 `apps.fas.usda.gov/opendata/api`. 공식 Swagger 스펙(`/opendata/swagger/docs/v1`)은 200으로 정상 서빙되고 우리 수집기 경로와 정확히 일치 — **코드 계약은 옳음, 이관 불필요**. ESR뿐 아니라 GATS 등 opendata 전 컨트롤러가 500 → 백엔드 전체 장애. 복구 감지: Swagger는 살아있으므로 `/api/esr/datareleasedates`가 200을 반환하면 복구된 것
